...
interface OidcAuthConfig {
  /** Provider authorization endpoint (e.g. Agentic360 IAM /oauth/v2/authorize). */
  authorizeUrl: string;
  clientId: string;
  redirectUri: string;
  /** Requested scopes; defaults to "openid email profile". */
  scope?: string;
  /** Opaque state passed through; the callback uses it to route the response. */
  state?: string;
  /** Button label, e.g. "Continue with Agentic360". */
  label?: string;
}
...
/** sessionStorage key holding the PKCE verifier between authorize and callback. */
export const OIDC_VERIFIER_KEY = "multica_oidc_verifier";
...
export function LoginPage({
  ...
  const handleOidcLogin = async () => {
    ...
    try {
      ...
      window.location.href = `${oidc.authorizeUrl}?${params}`;
    } catch {
            ...
            <>
              ...
              {oidc && (
                ...
                >
                  ...
                  {oidc.label || "Continue with Agentic360"}
                </Button>
              )}
